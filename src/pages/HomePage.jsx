...
function HomePage() {
  ...
  useEffect(() => {
    if (id != category) {
      //Al cambiar la categoría reiniciará el scroll, paginación y categoría
      setPagination({ ...pagination, ...{ limit: 40 } });
      setCategory(id);
      window.scrollTo(0, 0);
      return;
    }
    if (limit > 50) {
      //Si se pasa de la página 50 no hará la consulta
      return;
    }
    ...
    async function productsData() {
      ...
      const res = await fetch(
        `https://api.escuelajs.co/api/v1/${slug}?offset=0&limit=${limit}`
      );
